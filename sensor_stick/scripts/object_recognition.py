...
def pcl_callback(pcl_msg):
    ...
    LEAF_SIZE = 0.003 # used to be 0.01
    ...
    detected_objects = []
    print('cluster_incide size = ', len(cluster_indices))
    for index, pts_list in enumerate(cluster_indices):
        ...
        # print(prediction_confidence_list)

        prediction = clf.predict(scaler.transform(feature.reshape(1, -1)))
        print("prediction ", type(prediction), prediction,len(prediction))

        label = encoder.inverse_transform(prediction)[0]
        detected_objects_labels.append(label)
        prediction_confidence_list = clf.predict_proba(scaler.transform(feature.reshape(1, -1)))
        print(max(prediction_confidence_list))
...
if __name__ == '__main__':
    ...
    pcl_sub = rospy.Subscriber("/sensor_stick/point_cloud", pc2.PointCloud2, pcl_callback, queue_size=20)
